...
#define PROID		"产品的id"			//产品ID

#define AUTH_INFO	"创建设备时设置的鉴权信息"  //鉴权信息

#define DEVID		"设备的id"			//设备ID
		...
		if(result == 0)
			{
				...
				if(!json){
					...
					if(json_value->valueint){						
						Motor_SetSpeed(json_value->valueint);
						sprintf(buf,"speed:%d",json_value->valueint);
